...
class TestCaseRunner {
    constructor({ eventBroadcaster, eventDataCollector, stopwatch, gherkinDocument, newId, pickle, testCase, retries = 0, skip, supportCodeLibrary, worldParameters, }) {
    ...
    async run() {//Here is the loop that runs all of the steps
        for (let attempt = 0; attempt < this.maxAttempts; attempt++) {
            ...
            const scenariosMap = global.ALL_SCENARIOS || {};

            for (let i = 0; i < this.testCase.testSteps.length; i++) {//The LOOP!!!!!!
                const testStep = this.testCase.testSteps[i];
                const pickleStep = this.pickle.steps.find((pickleStep) => pickleStep.id === testStep.pickleStepId);

                if (pickleStep && scenariosMap[pickleStep.text]) {
                    // Inline the steps of the matched scenario
                    const scenarioToInline = scenariosMap[pickleStep.text];
                    console.log(`Inlining scenario: ${pickleStep.text} with steps:`, scenarioToInline.steps);
                    const newPickleSteps = scenarioToInline.steps.map((stepText, idx) => {
                        // Create a new pickleStep object
                        const id = `${pickleStep.id}_inlined_${idx}`;
                        const newPickleStep = {
                            id: id,
                            text: stepText,
                            type: 'Step',
                            argument: undefined,
                            astNodeIds: [id], // Assuming each step has a unique AST node ID
                            keyword: 'Then ', // You can set the keyword if you want (Given/When/Then)
                        };
                        // Add to pickle.steps so Cucumber can find it
                        this.pickle.steps.push(newPickleStep);
                        return newPickleStep;
                    });

                    // Create new testSteps for each step in the matched scenario
                    const newTestSteps = newPickleSteps.map((newPickleStep, idx) => {
                        return {
                            ...testStep,
                            id: `${testStep.id}_inlined_${idx}`,
                            pickleStepId: newPickleStep.id,
                            hookId: undefined // Make sure it's not a hook
                        };
                    });

                    // Replace the current step with the inlined steps
                    this.testCase.testSteps.splice(i, 1, ...newTestSteps);
                    // Move the index to the first new step (so the next iteration continues correctly)
                    // i += newTestSteps.length - 1;
                    i--; // Adjust index to account for the new steps
                    continue;
                }
            ...
            //#region // The orignal code that was here is commented out
            /**
              
            for (const testStep of this.testCase.testSteps) {//The LOOP!!!!!!
                const pickleStep = this.pickle.steps.find((pickleStep) => pickleStep.id === testStep.pickleStepId);
                if (pickleStep && isScenario(pickleStep.text)) {
                    //update the testCase ?
                    //run the pickle of the matches scenario
                }
                else await this.aroundTestStep(testStep.id, async () => {
                    if ((0, value_checker_1.doesHaveValue)(testStep.hookId)) {
                        const hookParameter = {
                            gherkinDocument: this.gherkinDocument,
                            pickle: this.pickle,
    ...
    async runStep(pickleStep, testStep) {//Here step runs!!!!
